...
function startPhase2() {
  ...
  paragraph.setAttribute(
    "aria-label",
    "Alinea 1. Geselecteerd. Druk op Enter om een notitie te maken.",
  );
  currentPhase = 2;
  updateTutorialMessage(
    "Goed zo! De alinea is nu geselecteerd. Druk op <kbd>Enter</kbd> om een notitie te maken.",
  );
}
...
function startPhase3() {
  ...
  currentPhase = 3;
  updateTutorialMessage(
    "Perfect! Het tekstvak is nu actief. Typ een korte notitie en druk op <kbd>Enter</kbd> om op te slaan.",
  );
}
...
document.addEventListener("keydown", (e) => {
  ...
  if (currentPhase === 4) {
    if (e.key.toLowerCase() === "n") {
      ...
      currentPhase = 5;
      updateTutorialMessage(
        'Geweldig! Je hebt je eerste notitie bekeken. Je kent nu de basis. <br><br> <span style="font-size: 0.9rem; font-weight: normal;">Je wordt nu doorgestuurd naar de echte tekst...</span>',
      );
...
form.addEventListener("submit", (e) => {
  ...
  currentPhase = 4;
  updateTutorialMessage(
    "Notitie opgeslagen! De tekst is weer beschikbaar. Druk nu op de letter <kbd>N</kbd> om naar je opgeslagen notities te gaan.",
  );
});
